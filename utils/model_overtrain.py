...
OUTPUT_DIR = "model_gpt2_scratch"
MAX_LEN = 2048  # We keep your wide window
    ...
    def __init__(self, tokenizer, data_dir, max_len=2048):
        ...
        for d in files:
            try:
                ...
                fam_key = meta['family'].lower()
                fam_token = "[FAM_WHISPER]" 
                if "stutter" in fam_key: fam_token = "[FAM_STUTTER]"
                elif "ghost" in fam_key: fam_token = "[FAM_GHOST]"
                elif "mute" in fam_key: fam_token = "[FAM_MUTE]"
                elif "binge" in fam_key: fam_token = "[FAM_BINGE]"
                elif "squeeze" in fam_key: fam_token = "[FAM_SQUEEZE]"
...
def train():
    ...
    # ✅ GPT-2 SMALL CONFIG (The Real Deal)
    config = GPT2Config(
        vocab_size=len(tokenizer),
        n_positions=2048, # Custom context
        n_embd=768,       # Standard GPT-2
    ...
    print(f"Model Parameters: {model.num_parameters() / 1e6:.2f}M") # ~124M
    ...
    args = TrainingArguments(
        ...
        num_train_epochs=10, 
        per_device_train_batch_size=2,  # ⚠️ 124M params + 2048 ctx is VRAM heavy. Reduce if OOM.
        gradient_accumulation_steps=4,  # Compensate for small batch size
        per_device_eval_batch_size=4,
        logging_steps=10,
        learning_rate=1e-4,             # Standard LR for this size
        save_strategy="no",
    ...
    print("--- Starting Full GPT-2 Scratch Training ---")
    trainer.train()
